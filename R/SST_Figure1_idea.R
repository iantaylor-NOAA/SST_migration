...
if(!exists("samps.all")){
  samps.all <- read.csv(file.path(mydir,
                                  'data/selected shortspine samples for migration study.csv'))
  samps.used <- read.csv(file.path(mydir,
...
if(!exists("catch.WCGBTS")){
  if(FALSE){
    ...
    catch.WCGBTS <- nwfscSurvey::PullCatch.fn(Name = "shortspine thornyhead",
                                              SurveyName = "NWFSC.Combo")
    bio.WCGBTS <- nwfscSurvey::PullBio.fn(Name = "shortspine thornyhead",
                                          SurveyName = "NWFSC.Combo")
...
for(iplot in 1:2){
  ...
  if(iplot == 2){
    plot(0, type = 'n', xlim=c(-126,-117),ylim=c(32,49), xaxs='i',yaxs='i',
         axes=FALSE, xlab="", ylab="")
  }
  ...
  contour(x=lonvec, y=latvec, z=t(d2), levels=c(-1000, -500, -100),
          drawlabels=FALSE, add=TRUE, lwd=1, col=contourColorVec)

  # add map
  if(iplot == 2){
    map('worldHires',xlim=c(-126,-117),ylim=c(32,49), xaxs='i',yaxs='i',
  ...
  if(iplot == 1){
    axis(2, at=axis2.vec, label=axis2.text, las=1)
                                        #axis(1, seq(-126,-117,2),paste(seq(126,117,-2),'°W', sep=''))
                                        #axis(2, seq(32,49,2),paste(seq(32,49,2),'°N', sep=''), las=1)
    abline(v = 500, lty = 3)
    # add region labels
    par(mgp = c(3,0.25,0))
    axis(4,
  ...
  if(iplot == 2){
    text(-124.1, 47.3, labels = "100m", cex = 0.7,
         col = contourColorVec[4], pos = 4) 
    arrows(-123.9, 47.3, -124.65, 47.1, length = 0.05,
           col = contourColorVec[4], lty = 1) 
    text(-125.1, 46.8, labels = "500m", cex = 0.7,
         col = contourColorVec[2], pos = 4) 
    text(-124.7, 46.5, labels = "1000m", cex = 0.7,
         col = contourColorVec[1], pos = 2)
  }
  # colors for each year
  yr.col.vec <- c(rgb(.8, 0,.5,1),
                  rgb( 1,.4, 0,1),
                  rgb( 1,.8, 0,1))
  yr.col.vec2 <- c(rgb(.8, 0,.5,1,maxColorValue=1.5),
  ...
  if(iplot == 1){
    ...
    points(x=samps$DEPTH_M, y=samps$HAUL_LATITUDE_DD,
           bg=samps$yr.col, col=samps$yr.col2, pch=21, cex=.7,)
  }
  if(iplot == 2){
    # add points where each haul is located in map
    points(x=samps$HAUL_LONGITUDE_DD, y=samps$HAUL_LATITUDE_DD,
           bg=samps$yr.col, col=samps$yr.col2, pch=21, cex=.7,)
  }
